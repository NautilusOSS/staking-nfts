...
interface ARC72ProviderRegisterOptions {
  apid: number;
  providerRate: number;
...
const providerRegister: any = async (options: ARC72ProviderRegisterOptions) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
    indexerClient,
  ...
  const providerRegisterR = await ci.provider_register(
    BigInt(options.providerRate),
    options.providerPayAddr
  );
  if (options.debug) {
    return providerRegisterR;
  }
  if (providerRegisterR.success) {
    if (!options.simulate) {
      await signSendAndConfirm(providerRegisterR.txns, sk);
    }
    return true;
  }
  return false;
...
arc72
  ...
  .requiredOption("-a, --apid <number>", "Specify the application ID")
  .requiredOption("-r, --provider-rate <number>", "Specify the provider rate")
  .requiredOption("-p, --provider-pay-addr <string>", "Specify the provider pay address")
  .option("-s, --simulate", "Simulate the provider register", false)
  .option("--debug", "Debug the provider register", false)
  .action(async (options: ARC72ProviderRegisterOptions) => {
    const providerRegisterR = await providerRegister(options);
    console.log("providerRegisterR:", providerRegisterR);
  });

interface ARC72DepositOptions {
  apid: number;
  tokenId: number;
  simulate?: boolean;
  debug?: boolean;
...
const deposit: any = async (options: ARC72DepositOptions) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
    indexerClient,
  ...
  ci.setFee(2000);
  ci.setPaymentAmount(Number(options.amount) * 1e6);
  const depositR = await ci.deposit(BigInt(options.tokenId));
  if (options.debug) {
    return depositR;
  }
  if (depositR.success) {
...
arc72
  .command("deposit")
  .description("Deposit to the arc72 token")
  .requiredOption("-a, --apid <number>", "Specify the application ID")
  .requiredOption("-t, --token-id <number>", "Specify the token ID")
  .requiredOption("-m, --amount <number>", "Specify the amount")
  .option("-s, --simulate", "Simulate the deposit", false)
  .option("--debug", "Debug the deployment", false)
  .action(async (options: ARC72DepositOptions) => {
    const depositR = await deposit(options);
    console.log("depositR:", depositR);
  });

interface ARC72SyncMabOptions {
  apid: number;
  tokenId: number;
  simulate?: boolean;
  debug?: boolean;
...
const syncMab: any = async (options: ARC72SyncMabOptions) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
    indexerClient,
  ...
  const syncMabR = await ci.sync_mab(BigInt(options.tokenId));
  if (options.simulate) {
    return syncMabR;
  }
  if (syncMabR.success) {
    if (!options.simulate) {
      await signSendAndConfirm(syncMabR.txns, sk);
    }
    return true;
  }
  return false;
...
arc72
  .command("sync-mab")
  .description("Sync the mab of the arc72 token")
  .requiredOption("-a, --apid <number>", "Specify the application ID")
  .requiredOption("-t, --token-id <number>", "Specify the token ID")
  .option("-s, --simulate", "Simulate the sync", false)
  .option("--debug", "Debug the deployment", false)
  .action(async (options: ARC72SyncMabOptions) => {
    const syncMabR = await syncMab(options);
    console.log("syncMabR:", syncMabR);
  });

interface ARC72MabOptions {
  apid: number;
  tokenId: number;
}

const mab: any = async (options: ARC72MabOptions) => {
  const ci = new CONTRACT(
    Number(options.apid),
    algodClient,
    indexerClient,
  ...
  const mabR = await ci.mab(BigInt(options.tokenId));
  if (mabR.success) {
    return mabR.returnValue.toString();
  }
  return "0";
};
...
arc72
  .command("mab")
  .description("Get the mab of the arc72 token")
  .requiredOption("-a, --apid <number>", "Specify the application ID")
  .requiredOption("-t, --token-id <number>", "Specify the token ID")
  .action(async (options: ARC72MabOptions) => {
    const mabR = await mab(options);
    console.log("mab:", mabR);
  });
