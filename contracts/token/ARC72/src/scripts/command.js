...
const providerRegister = async (options) => {
    const ci = new CONTRACT(Number(options.apid), algodClient, indexerClient, makeSpec(OSARC72TokenSpec.contract.methods), {
        addr: addr,
        sk: sk,
    });
    const providerRegisterR = await ci.provider_register(BigInt(options.providerRate), options.providerPayAddr);
    if (options.debug) {
...
arc72
    ...
    const metadata = options.metadata
        ? new Uint8Array(Buffer.from(padStringWithZeroBytes(options.metadata, 256), "utf8"))
        : new Uint8Array(256);
